...
### Risks & Mitigations
- Node/Python version mismatch → pin versions in .tool-versions or README
...
- WorkflowContext manages stage transitions: config → scenes → assets → assembly → output
...
## Work Package WP03: Configuration UI - User Story 2 & 3 (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Invalid API key UX → show inline validation errors with retry option

---

## Work Package WP04: LLM Streaming Service & Scene Generation UI - User Story 1 (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Malformed JSON from LLM → graceful parse error with retry option (FR-035)
- DeepSeek CORS issues → fallback to OpenAI or show error

---

## Work Package WP05: Image Generation Service (Priority: P1) 🎯 MVP
...
- Memory usage → release canvas resources after processing

---

## Work Package WP06: TTS Service with Context (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- TTS timeout → retry with exponential backoff (FR-026)
- API quota exhausted → clear error message (FR-035)

---

## Work Package WP07: Asset Generation UI - User Story 4 (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Partial failures → ensure completed assets preserved, only failed ones retry

---

## Work Package WP08: Backend Video Assembly (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- FFmpeg not found → ensure in Dockerfile, provide clear error
- Timeout on long videos → set 300s timeout in Cloud Run config

---

## Work Package WP09: Video Assembly Client & Output UI - User Story 6 (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Large video transfer → stream response, show progress
- Backend unavailable → clear error message with retry option (FR-035)
...
### Risks & Mitigations
- Regression risk → manual smoke test through full workflow
...
### Risks & Mitigations
- GCP credentials exposure → use GitHub encrypted secrets
- CORS issues in production → verify Cloud Run URL in frontend CORS config

---

## Dependency & Execution Summary

- **Sequence**: WP01 → WP02 → WP03/WP04 (can parallel) → WP05/WP06 (can parallel) → WP07 → WP08 (can start with WP04) → WP09 → WP10 → WP11
- **Parallelization**:
